...
@onready var slotsHotbar : Array = $NinePatchRect/HBoxContainer.get_children()
@onready var slotsInventory : Array = slotsHotbar + $NinePatchRect/InventoryContainer.get_children()
...
func update():
	...
	for i in range(min(inventory.slots.size(), slotsInventory.size())):
		var inventorySlot : InventorySlot = inventory.slots[i]
		
		if !inventorySlot.item : continue
		
		var itemStackGui : ItemStackGUI = slotsInventory[i].itemStackGui
		if !itemStackGui:
			itemStackGui = ItemStackGUIClass.instantiate()
			slotsInventory[i].insert(itemStackGui)
...
func putItemBack():
	locked = true
	if oldIndex < 0:
		var emptySlots = slotsInventory.filter(func (s) : return s.isEmpty())
		if emptySlots.is_empty() : return
		
		oldIndex = emptySlots[0].index
		
	var targetSlot = slotsInventory[oldIndex]
